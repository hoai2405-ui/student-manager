...
function parseMysqlDateToLocal(dateVal) {
...
async function getStudentCourseAccess(studentId) {
  const [[row]] = await pool.query(
    `SELECT
       ...
       c.so_ngay_hoc
     FROM students s
     LEFT JOIN courses c ON c.ma_khoa_hoc = s.ma_khoa_hoc
     WHERE s.id = ?
     LIMIT 1`,
    [studentId]
  );
  ...
  const start = parseMysqlDateToLocal(row.ngay_hoc);
  const totalDays = Number(row.so_ngay_hoc || 0);

  // Nếu khóa học chưa cấu hình thời lượng thì không chặn.
  ...
  const makeupFrom = toValidDate(row.makeup_from);
  const makeupTo = toValidDate(row.makeup_to);
  const inMakeup =
    makeupFrom &&
    makeupTo &&
    now.getTime() >= makeupFrom.getTime() &&
    now.getTime() <= makeupTo.getTime();

  return {
    allowed: !expired || Boolean(inMakeup),
    expired,
    course: {
      ma_khoa_hoc: row.ma_khoa_hoc || null,
      ten_khoa_hoc: row.ten_khoa_hoc || null,
      ngay_hoc: row.ngay_hoc || null,
      so_ngay_hoc: totalDays,
      end_at: end.toISOString(),
    },
    makeup: {
      makeup_from: makeupFrom ? makeupFrom.toISOString() : null,
      makeup_to: makeupTo ? makeupTo.toISOString() : null,
      in_makeup: Boolean(inMakeup),
    },
  };
}

const enforceStudentCourseAccess = async (req, res, next) => {
  try {
    // Admin/staff bypass
    if (
      req.user?.is_admin === 1 ||
    ...
    const access = await getStudentCourseAccess(studentId);
    if (access.allowed) return next();

    return res.status(403).json({
      message: "Khóa học đã kết thúc",
      code: "COURSE_EXPIRED",
      ...access,
    });
  } catch (e) {
    return res
      .status(500)
      .json({ message: "Lỗi kiểm tra thời hạn khóa học", error: e.message });
  }
};
...
async function createTables() {
  try {
    ...
    // Ensure slot_id/status columns exist for older DBs
    try {
      await pool.query(
        "ALTER TABLE registrations ADD COLUMN slot_id VARCHAR(32) DEFAULT NULL"
      );
    } catch (e) {
      // ignore
    }
    ...
    try {
      await pool.query(
        "ALTER TABLE registrations ADD COLUMN status ENUM('active','cancelled') DEFAULT 'active'"
      );
    } catch (e) {
      // ignore
    }
    ...
    // Học bù (mở lại thời gian học sau khi hết hạn khóa)
    try {
      await pool.query("ALTER TABLE students ADD COLUMN makeup_from DATETIME NULL");
    } catch (e) {
      // ignore
    }
    try {
      await pool.query("ALTER TABLE students ADD COLUMN makeup_to DATETIME NULL");
    } catch (e) {
      // ignore
    }
...
// Admin: mở khoảng thời gian học bù (cho phép học viên học lại / mở khóa tính giờ)
app.post(
  "/api/students/makeup-window",
  authenticateToken,
  ...
  async (req, res) => {
    ...
    try {
      ...
      const [[s]] = await pool.query("SELECT id FROM students WHERE id = ? LIMIT 1", [studentId]);
      if (!s) return res.status(404).json({ message: "Học viên không tồn tại" });

      await pool.query(
        "UPDATE students SET makeup_from = ?, makeup_to = ? WHERE id = ?",
        [fromDate, toDate, studentId]
      );

      res.json({ success: true });
    } catch (e) {
      res.status(500).json({ message: "Lỗi mở học bù", error: e.message });
    }
  }
);
...
app.post("/api/schedules/:id/register", authenticateToken, checkAdmin, async (req, res) => {
  try {
    ...
    // Điều kiện: hoàn thành TỐI THIỂU 5 môn có yêu cầu giờ học.
    // Trước đây dùng Math.max(5, requiredRows.length) => nếu có >5 môn yêu cầu,
    // sẽ bắt học viên hoàn thành hết (khắt khe hơn nghiệp vụ), dễ gây trường hợp
    // "tiến độ đã đạt" nhưng vẫn bị chặn đăng ký.
    ...
    if (err && err.code === "ER_DUP_ENTRY") {
      return res
        .status(400)
        .json({ message: "Học viên đã đăng ký lịch này", error: "Already registered" });
    }
    console.error(err);
    res.status(500).json({ error: err.message });
  }
...
const groupScheduleRegistrations = (rows) => {
  const groupedData = rows.reduce((acc, row) => {
    ...
    acc[key].selected_slots.push({
      ...
      period: start ? (start.getHours() < 12 ? "morning" : "afternoon") : null,
...
// Admin: Lấy danh sách tất cả đăng ký lịch học
app.get(
  ...
  async (req, res) => {
    try {
      ...
      SELECT
        ...
        s.anh_chan_dung as student_avatar,
    ...
    `);
      ...
      res.json(groupScheduleRegistrations(rows));
    } catch (err) {
...
// Student: Lấy lịch đã đăng ký của chính mình
app.get(
  "/api/student/schedule-registrations",
  authenticateToken,
  async (req, res) => {
    try {
      const studentId = req.user?.id;
      if (!studentId) return res.status(401).json({ message: "Thiếu token xác thực" });
      ...
      console.error("Error fetching student schedule registrations:", err);
      res.status(500).json({
        message: "Lỗi lấy lịch đã đăng ký",
        error: err.message,
      });
    }
  }
...
app.get("/api/subject-requirements", async (req, res) => {
  try {
    ...
    const [rows] = await pool.query(
      "SELECT * FROM subject_requirements WHERE subject_id = ? ORDER BY license_class",
      [subject_id]
    );
...
app.post(
  ...
  requireAdminOrDepartment,
  async (req, res) => {
    const { code, name, total_hours = 0, description = null } = req.body || {};
    ...
    try {
      ...
      if (dup) return res.status(409).json({ message: "Mã môn đã tồn tại" });

      const [result] = await pool.query(
        "INSERT INTO subjects (code, name, total_hours, description) VALUES (?, ?, ?, ?)",
        [String(code).trim(), String(name).trim(), Number(total_hours) || 0, description]
      );
      res.json({ success: true, id: result.insertId });
    } catch (e) {
      res.status(500).json({ message: "Lỗi tạo môn học", error: e.message });
    }
  }
);
...
app.put(
  ...
  async (req, res) => {
    ...
    if (!subjectId) return res.status(400).json({ message: "id không hợp lệ" });
    if (!code || !String(code).trim()) return res.status(400).json({ message: "Thiếu code" });
    if (!name || !String(name).trim()) return res.status(400).json({ message: "Thiếu name" });

    try {
      const [[dup]] = await pool.query(
        "SELECT id FROM subjects WHERE code = ? AND id <> ? LIMIT 1",
        [String(code).trim(), subjectId]
      );
      if (dup) return res.status(409).json({ message: "Mã môn đã tồn tại" });

      await pool.query(
        "UPDATE subjects SET code = ?, name = ?, total_hours = ?, description = ? WHERE id = ?",
        [String(code).trim(), String(name).trim(), Number(total_hours) || 0, description, subjectId]
      );
      res.json({ success: true });
    } catch (e) {
      res.status(500).json({ message: "Lỗi cập nhật môn học", error: e.message });
    }
  }
);
...
app.delete(
  ...
  async (req, res) => {
    ...
    try {
      ...
      res.status(500).json({ message: "Lỗi xóa môn học", error: e.message });
    }
  }
);

// Admin: bulk upsert requirements for a subject
app.put(
  "/api/admin/subjects/:id/requirements",
  authenticateToken,
  requireAdminOrDepartment,
  async (req, res) => {
    const subjectId = Number(req.params.id);
    ...
    const normalized = items
      .map((r) => ({
        ...
        license_class: String(r?.license_class || "").trim(),
        required_hours: Number(r?.required_hours || 0) || 0,
      }))
      .filter((r) => r.license_class);

    try {
      const [[sub]] = await pool.query("SELECT id FROM subjects WHERE id = ? LIMIT 1", [subjectId]);
      if (!sub) return res.status(404).json({ message: "Môn học không tồn tại" });

      // Replace all requirements for this subject (simple + predictable)
      await pool.query("DELETE FROM subject_requirements WHERE subject_id = ?", [subjectId]);

      for (const r of normalized) {
        await pool.query(
          "INSERT INTO subject_requirements (subject_id, required_hours, license_class) VALUES (?, ?, ?)",
          [subjectId, r.required_hours, r.license_class]
        );
      }

      res.json({ success: true });
    } catch (e) {
      res.status(500).json({ message: "Lỗi lưu yêu cầu môn học", error: e.message });
    }
  }
);
...
app.post("/api/student/login", async (req, res) => {
  try {
    ...
    const access = await getStudentCourseAccess(student.id);
    if (!access.allowed) {
      return res.status(403).json({
        message: "Khóa học đã kết thúc",
        code: "COURSE_EXPIRED",
...
app.get("/api/subjects", authenticateToken, enforceStudentCourseAccess, async (req, res) => {
  try {
...
app.get("/api/lessons", authenticateToken, enforceStudentCourseAccess, async (req, res) => {
...
app.get("/api/lessons/:id", authenticateToken, enforceStudentCourseAccess, async (req, res) => {
...
app.post("/api/progress/save", authenticateToken, enforceStudentCourseAccess, async (req, res) => {
...
// Lấy tiến độ 1 bài
app.get("/api/progress/:lessonId", authenticateToken, enforceStudentCourseAccess, async (req, res) => {
    const studentId = req.user.id;
...
async function requireAdminOrDepartment(req, res, next) {
  try {
